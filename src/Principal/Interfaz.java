...
public class Interfaz extends javax.swing.JFrame {
    ...
    private HashSet<String> cjtoEstados = new HashSet(); // Conjunto de datos utilizando tablas Hash que no permite duplicados, eficiente en manejo
    private HashSet<String> cjtoSimbolos = new HashSet();
    private DefaultTableModel modeloTT; // Es una clase en Java que forma parte de la biblioteca Swing, utilizada para representar datos en una tabla (JTable).
    ...
    private String estadoActual = ""; // Estado evaluar
    private Stack<HashSet<String>> ultimosEstados = new Stack(); // Pila de cjtos de estados
    private HashSet<String> estadosActuales = new HashSet(); // Estados a evaluar
    ...
    public void actualizarTabla() {
        ...
        ArrayList<Object> filaTransicion = new ArrayList<>();

        // Agregar encabezados de columnas
        filaTransicion.add("ESTADOS/SIMBOLOS");
        Collections.addAll(filaTransicion, simbolos);

        // Agregar columna de símbolo LAMBDA si es AFND
        if (tipoAFND.isSelected()) {
            filaTransicion.add("L");
        }
        modeloTT.setColumnIdentifiers(filaTransicion.toArray());

        // Rellenar las filas de la tabla
        for (String estado : cjtoEstados) {
            filaTransicion.clear(); // Reiniciar la fila
            filaTransicion.add(estado);

            if (tipoAFD.isSelected()) {
                for (int j = 0; j < cjtoSimbolos.size(); j++) {
                    char simbolo = tablaTransicion.getColumnName(j + 1).charAt(0);
                    filaTransicion.add(afd.getTransicion(estado, simbolo));
                }
            } else {
                for (int j = 0; j < cjtoSimbolos.size() + 1; j++) {
                    String nombreColumna = tablaTransicion.getColumnName(j + 1);
                    if (nombreColumna.equals("L")) {
                        filaTransicion.add(afnd.getTransicionL(estado));
                    } else {
                        char simbolo = nombreColumna.charAt(0);
                        filaTransicion.add(afnd.getTransicion(estado, simbolo));
                    }
                }
            }

            modeloTT.addRow(filaTransicion.toArray());
        }
    }
    ...
    private void initComponents() {
        ...
        setTitle("Automata AFN y AFD Grupo 2");
        setBackground(new java.awt.Color(255, 255, 255));
        setLocation(new java.awt.Point(0, 0));

        jLabel1.setFont(new java.awt.Font("Rockwell", 0, 12)); // NOI18N
        jLabel1.setText("Automata AFN y AFD Grupo 2");
        ...
        layout.setHorizontalGroup(
                layout.createSequentialGroup() // Cambiado a SequentialGroup
                        .addContainerGap()
    ...
    private void tipoAFNDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tipoAFNDActionPerformed
        ...
        comboSimbolo.removeItem("Símbolo");
    ...
    private void botonAddTActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonAddTActionPerformed
        if(Objects.equals(comboSimbolo.getSelectedItem().toString(), "Símbolo")){
            return;
        }
        if(Objects.equals(comboOrigen.getSelectedItem().toString(), "Origen")){
            return;
        }
        if(Objects.equals(comboDestino.getSelectedItem().toString(), "Destino")){
            return;
        }
    ...
    private void botonEliminarTActionPerformed(java.awt.event.ActionEvent evt) {
        if (tipoAFD.isSelected()) {
            // Manejo para AFD
            for (int i : tablaTransicion.getSelectedRows()) {
                ...
                for (int j = 1; j < modeloTT.getColumnCount(); j++) {
                    ...
                    System.out.println("Eliminar transición " + t);
                    afd.eliminarTransicion(t); // Elimina la transición
                }
            }
        } else { // Manejo para AFND
            for (int i : tablaTransicion.getSelectedRows()) {
                System.out.println("Eliminando fila AFND: " + i);
                String origen = modeloTT.getValueAt(i, 0).toString();

                for (int j = 1; j < modeloTT.getColumnCount(); j++) {
                    String simbolo = modeloTT.getColumnName(j);

                    if (simbolo.equals("L")) { // Si es una transición lambda
                        String destino = modeloTT.getValueAt(i, j).toString();
                        TransicionL tL = new TransicionL(origen, stringToHashSet(destino));
                        System.out.println("Eliminar transición lambda " + tL);
                        afnd.eliminarTransicionL(tL);
                    } else { // Transición normal
                        String destino = modeloTT.getValueAt(i, j).toString();
                        TransicionAFND t = new TransicionAFND(origen, simbolo.charAt(0), stringToHashSet(destino));
                        System.out.println("Eliminar transición AFND " + t);
                        afnd.eliminarTransicion(t);
                    }
                }
            }
        }
    ...
    private HashSet<String> stringToHashSet(String input){
        ...
        // Step 3: Create a HashSet and add trimmed elements
        HashSet<String> hashSet = new HashSet<>();
        for (String element : elements) {
            hashSet.add(element.trim()); // Trim whitespace
        }
